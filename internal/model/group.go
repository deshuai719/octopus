...
type GroupSessionKeepMode string

const (
	GroupSessionKeepModeTTL          GroupSessionKeepMode = "ttl"
	GroupSessionKeepModeUntilFailure GroupSessionKeepMode = "until_failure"
)

func NormalizeGroupSessionKeepMode(mode GroupSessionKeepMode) GroupSessionKeepMode {
	switch mode {
	case GroupSessionKeepModeUntilFailure:
		return GroupSessionKeepModeUntilFailure
	default:
		return GroupSessionKeepModeTTL
	}
}

func (g Group) EffectiveSessionKeepMode() GroupSessionKeepMode {
	return NormalizeGroupSessionKeepMode(g.SessionKeepMode)
}

func (g Group) SessionKeepsUntilFailure() bool {
	return g.EffectiveSessionKeepMode() == GroupSessionKeepModeUntilFailure
}
...
type Group struct {
	...
	SessionKeepMode       GroupSessionKeepMode `json:"session_keep_mode" gorm:"type:varchar(32);not null;default:'ttl'"`
	...
	PaidSiteLowRatioFirst bool                 `json:"paid_site_low_ratio_first" gorm:"default:false"`
...
type GroupItem struct {
	...
	PaidSite       bool     `json:"-" gorm:"-"`
	SiteGroupRatio *float64 `json:"-" gorm:"-"`
}
...
type GroupPreset struct {
	...
	PaidSiteLowRatioFirst bool                 `json:"paid_site_low_ratio_first"`
...
type GroupUpdateRequest struct {
	...
	SessionKeepMode       *GroupSessionKeepMode    `json:"session_keep_mode,omitempty"`
	RetryEnabled          *bool                    `json:"retry_enabled,omitempty"` // 启用同通道重试+透传429/503
	MaxRetries            *int                     `json:"max_retries,omitempty"`   // 同通道最大重试次数
	PaidSiteLowRatioFirst *bool                    `json:"paid_site_low_ratio_first,omitempty"`
